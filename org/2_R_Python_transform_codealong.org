...
- Use the flag vector to index the dataframe, remove the record and
  ...
  #+begin_src R
    df$ID[idx]  # value corresponding to the idx condition
    which(idx) # indices corresponding to the idx condition
    df <- df[-which(idx),] # returns all rows for all columns except idx1
    df
  #+end_src
...
- The 'ID' column is not statistically relevant so let's remove it
  by selecting only the second and third column and overwriting ~df~:
  #+begin_src python
...
- Compare with R's =summary=:
  ...
  3. remove first column (no statistical value)
  4. print =summary= of data:
  #+begin_src R
    df <- 
    summary(df)
  #+end_src
  ...
  :        ID           Test.1          Test.2     
  :  Min.   : 646   Min.   : 4.83   Min.   : 3.50  
  :  1st Qu.:1295   1st Qu.:15.66   1st Qu.: 9.50  
  :  Median :1433   Median :16.83   Median :10.33  
  :  Mean   :1407   Mean   :15.50   Mean   :10.35  
  :  3rd Qu.:1488   3rd Qu.:17.41   3rd Qu.:11.33  
  :  Max.   :2157   Max.   :19.07   Max.   :14.50  
  :                 NA's   :2
