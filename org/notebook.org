...
** DONE New bonus assignment "Nesting complex if statements" (Canvas)
** DONE New bonus assignment "Complex while loop problem" (Canvas)
** DONE Review =switch=
...
1) Problem:
   - Calculate the average flow of the =Nile= River until you encounter
     a year where the flow is less than 900 (million cubic meters).
...
2) Solution in R:
   #+begin_src R
     ...
     ## Loop through dataset until a flow less than 900 is found
     while (Nile[i] >= 900) {
     ...
     ## print results
     cat("Years until encountering a flow less than 900:", year_count, "\n")
